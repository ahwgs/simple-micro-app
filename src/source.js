...
const extractSourceDom = (parent, app) => {
...
export const loadHtml = (app) => {
  ...
  fetchSource(app.url).then((html) => {
    ...
    console.log("fetchLinksFromHtml", app);
...
const fetchLinksFromHtml = (app, htmlDom) => {
...
const fetchScriptsFromHtml = (app, htmlDom) => {
  const scriptEntries = Array.from(app.source.scripts.entries());
  // 通过fetch请求所有js资源
  const fetchScriptPromise = [];
  for (const [url, info] of scriptEntries) {
    // 如果是内联script，则不需要请求资源
    // 只有内联的有code 这是在拉html 处理函数里面set的
